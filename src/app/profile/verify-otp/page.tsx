...
const VerifyOTP = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await axios.post(`${API_URL}/users/verify-forgot-password`, { otp: otpValue }, { withCredentials: true });

            const savedData = localStorage.getItem('pendingResetPassword');
            if (!savedData) throw new Error("No pending password data found.");

            const { email, newPassword, confirmPassword } = JSON.parse(savedData);
    ...
    return (
        <div className="flex items-center justify-center md:h-screen md:mt-0 mt-14 w-full z-20">
            ...
            <div className="bg-white dark:bg-dark-500 rounded-xl shadow-md md:py-9 py-[30px] md:px-[50px] px-5 md:space-y-5 space-y-3">
                <h2 className="md:text-2xl text-xl font-bold text-neutral-500 dark:text-whitish-500">Verify OTP</h2>
                ...
                <div className="flex items-center gap-2 py-2">
                    <Back />
                    <a href="/profile" className="text-sm font-semibold text-secondary-400 hover:underline">
                        Back to profile
                    </a>
                </div>
            </div>
